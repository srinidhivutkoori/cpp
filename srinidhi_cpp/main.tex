...
srinidhi.vutkoori@student.ncirl.ie}}
...
The data layer uses Amazon DynamoDB with three tables: a conferences table keyed by conferenceId, a papers table keyed by paperId with a global secondary index on conferenceId for efficient queries of all papers within a conference, and a users table keyed by userId for authentication data. DynamoDB was selected over relational alternatives because the primary access patterns involve retrieving individual entities by their identifier or querying papers by conference, both of which align efficiently with DynamoDB's key-value and secondary index query patterns \cite{awsdynamodb}. The PDF documents are stored in a dedicated S3 bucket with presigned URLs generated for secure, time-limited download access.
...
Amazon DynamoDB provides a fully managed NoSQL database for storing conference, paper, and user data across three dedicated tables \cite{awsdynamodb}. The tables use on-demand billing mode, which is ideal for conference systems where write volume spikes during submission windows and drops to near zero between conferences. DynamoDB was preferred over Amazon RDS because the application's access patterns are predominantly key-value lookups and single-table queries that do not require complex relational joins. The papers table employs a Global Secondary Index on the conferenceId attribute to enable efficient retrieval of all papers submitted to a specific conference without performing a full table scan.
...
The implementation follows a clean separation between the frontend React application, the serverless Lambda backend with JWT-based authentication, and the custom Python library. This section documents the implementation of each major component.

\subsection{Backend API and CRUD Operations}

The Lambda function implements role-aware API routes through a path-based routing mechanism. All authenticated endpoints extract the JWT token from the Authorization header, decode it to obtain the user's role and identity, and enforce role-based access before processing the request. Table~\ref{tab:endpoints} summarises the primary API endpoints.
...
A critical implementation detail involved the role-based access control enforcement within the JWT middleware. The system validates not only that the token is present and properly signed, but also that the decoded role matches the required role for each endpoint. The status transition validation leverages the confmanager-nci library to ensure that only valid workflow transitions are permitted:

\begin{lstlisting}[language=Python, caption={JWT role validation and status transition}]
import jwt
from confmanager import PaperManager

def validate_role(token, required_role):
    payload = jwt.decode(token, SECRET, algorithms=["HS256"])
    if payload["role"] != required_role:
        return None
    return payload

def update_paper_status(paper_id, new_status, token):
    user = validate_role(token, "admin")
    if not user:
        return {"statusCode": 403, "body": "Forbidden"}
    paper = get_paper(paper_id)
    mgr = PaperManager()
    if not mgr.validate_transition(
            paper["status"], new_status):
        return {"statusCode": 400,
                "body": "Invalid status transition"}
    # Update status and notify via SNS
    update_status_in_db(paper_id, new_status)
    notify_status_change(paper, new_status)
...
The confmanager-nci library includes 43 automated unit tests implemented with pytest. The tests validate paper submission with boundary conditions on title length and abstract word count, review score validation at boundary values of one and ten, all permitted and prohibited status transitions in the paper workflow state machine, conference deadline enforcement including edge cases at exact deadline times, and formatter output consistency across all supported formats. The CI/CD pipeline executes this test suite as a mandatory gate before deployment proceeds.
...
The first job, \texttt{test-library}, installs the confmanager-nci library in editable mode and executes the full pytest suite of 43 unit tests. Only when all tests pass does the pipeline proceed to backend deployment.

The second job, \texttt{deploy-backend}, provisions all required AWS infrastructure using the AWS CLI. This includes creating three DynamoDB tables (conferences, papers, users) with on-demand billing and appropriate key schemas, creating S3 buckets with CORS configurations for PDF storage and frontend hosting, creating an IAM role with policies scoped to the specific DynamoDB tables, S3 buckets, SNS topic, and CloudWatch log groups, creating an SNS topic for status change notifications, packaging the Lambda function code with all dependencies including the confmanager-nci library and the PyJWT package, and configuring API Gateway with proxy integration. All resource creation commands are idempotent, using existence checks to handle repeated deployments safely.
...
Several key findings emerged during the development process. First, implementing role-based access control through JWT claims proved effective for a serverless architecture where traditional session-based authentication is impractical. By encoding the user's role directly in the token, the Lambda function can make authorisation decisions without additional database lookups on every request. However, this approach requires careful token expiration management and would benefit from refresh token rotation for production deployments.
...
Third, DynamoDB's Global Secondary Index on the papers table proved valuable for the common query pattern of retrieving all papers for a specific conference. Without this index, each conference view would require a full table scan, which would become increasingly expensive as the number of submitted papers grows across multiple conferences.
